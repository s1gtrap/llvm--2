...
int64_t exponent(int64_t base, int64_t expn)
{
  if (expn < 0)
  {
    return 0; // Integers can't handle fractions. Round to zero rather than report an error.
  }
  else if (expn == 0)
  {
    return 1;
  ...
  {
    return base * exponent(base, expn - 1);
  }
}

void dummy(void *static_link, struct string *a, struct string *b, struct string *c, struct string *d, struct string *e, struct string *f, struct string *g, struct string *h, struct string *i, struct string *j)
{
  print(static_link, a);
  printf("\n");
  print(static_link, b);
