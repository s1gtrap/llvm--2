...
let str_ s ppy _ = Fmt.string ppy s
let sym_ s = str_ (Symbol.name s)
let int_ i fmt _ = Fmt.int fmt i
let colon : unit Fmt.t = str_ ":"

exception NotImplemented
exception Impossible

let rec pp_ty ?(unfold = false) ty =
  let open Types in
  let open Fmt in
  match ty with
  | NIL -> str_ "nil"
...
let rec pp_exp' ?(inlet = false) (Exp { exp_base; ty; _ }) =
  let open Fmt in
  match exp_base with
  | IntExp x -> int_ x
  | NilExp -> str_ "nil"
  ...
  | IfExp { test; thn; els; _ } ->
      let pp_els =
        match els with
        | Some els -> concat ~sep:sp [ str_ "else"; pp_exp' els ]
        | None -> nop
      in
      let box1 =
        [ str_ "if"; pp_exp' test ] |> concat ~sep:sp |> hvbox ~indent:2
      in
      let box2 =
        [ str_ "then"; pp_exp' thn ] |> concat ~sep:sp |> hvbox ~indent:2
      in
  ...
  | ArrayExp { size; init; _ } ->
      ...
      [ pp_t; brackets (pp_exp' size); str_ " of "; pp_exp' init ]
      |> concat |> hvbox
  | RecordExp { fields; _ } ->
  ...
  | SeqExp exps ->
      List.map pp_exp' exps |> concat ~sep:semi |> hvbox
      |> if inlet then id else parens
...
let pp_fdecl (Fdecl { name; args; result = r; body; _ }) =
  let open Fmt in
  let pp_arg (Arg { name; ty; _ }) = hbox @@ (sym_ name ++ colon ++ pp_ty ty) in
  let args = List.map pp_arg args |> concat ~sep:comma |> parens in
  let ty' = if r = Types.VOID then nop else colon ++ pp_ty r in
  concat
    [
      [ str_ "function "; sym_ name; args; ty'; str_ " =" ]
      |> concat ~sep:nop |> hvbox;
      pp_exp body;
    ]
  |> vbox ~indent:2

let pp_tdecl (Tdecl { name; ty; _ }) =
  let open Fmt in
