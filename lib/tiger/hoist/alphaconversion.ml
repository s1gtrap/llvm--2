...
module TypeMap = Map.Make (struct
  ...
  let compare a b =
    ...
    | Ty.NAME (s, _), Ty.NAME (s', _) -> compare s s'
    | Ty.RECORD (_, u), Ty.RECORD (_, u') -> compare u u'
...
let rec alpha_type ({typemap; _} as ctxt : context) (ty : Ty.ty) : Ty.ty =
  ...
  | None -> (
    ...
    | Ty.RECORD (fields, uq) ->
        let ty' =
          Ty.RECORD
            (List.map (fun (s, ty) -> (s, alpha_type ctxt ty)) fields, uq)
        in
...
let rec alpha_exp (ctxt : context) (Exp {exp_base; pos; ty} : T.exp) : A.exp
  ...
  match exp_base with
  | T.VarExp x ->
      let v, v_ty = v_ty @@ alpha_var ctxt x in
      A.VarExp v ^! v_ty
  | T.NilExp -> A.NilExp ^! Ty.NIL
  | T.IntExp n -> A.IntExp (Int64.to_int n) ^! Ty.INT
  ...
  | T.SeqExp exps -> (
      ...
      match exps' with
      | [] -> Ty.VOID
      | _ -> List.rev exps' |> List.hd |> e_ty |> snd )
  | T.AssignExp {var; exp} ->
      A.AssignExp {var= alpha_var ctxt var; exp= alpha exp} ^! Ty.VOID
  | T.IfExp {test; thn; els} ->
      A.IfExp {test= alpha test; thn= alpha thn; els= Option.map alpha els}
      ^! trans_ty ctxt ty
  ...
  | T.ArrayExp {size; init} ->
      A.ArrayExp {size= alpha size; init= alpha init} ^! trans_ty ctxt ty
...
and alpha_dec (ctxt : context) (dec : T.decl) : context * A.decl =
  ...
  | FunctionDec fdecs ->
      ...
      let fdecs =
        List.map
          (fun (T.Fdecl {name; args; result; body; pos}) ->
            let venv', args =
              List.fold_left_map
                (fun venv (T.Arg {name; escape; ty; pos}) ->
...
let from_string_changed ls acc =
  List.fold_left
    (fun t (s, s') ->
      let sym = S.symbol s and sym' = S.symbol s' in
      S.enter (t, sym, sym') )
    acc ls
...
let alpha_convert (texp : T.exp) : A.exp =
  let venv =
    ...
    |> from_string_unchanged
         ...
         ; "not"
         ; "dummy" ]
    |> from_string_changed [("getchar", "getChar"); ("exit", "tigerexit")]
  in
  let tenv = S.empty |> from_string_unchanged ["int"; "string"] in
  let typemap =
    ref
    @@ List.fold_left
         (fun m t -> TypeMap.add t t m)
         TypeMap.empty
         [Ty.INT; Ty.STRING; Ty.NIL; Ty.VOID]
  in
  alpha_exp {venv; tenv; fresh_counter= ref 0; typemap} texp
