...
let field (Arg {name; escape; _}, d) =
  concat
    [indent d; "("; string_of_symbol name; ","; string_of_bool !escape; ")"]
...
let as_string max_unfolds e0 =
  ...
  and exp_desc (e, d) =
    ...
    | VarExp v ->
        ...
        var (v, d + 1) ;
        add ")"
    | IntExp i -> add @@ concat ["IntExp("; Int64.to_string i; ")"]
    | NilExp -> add "NilExp"
    ...
    | RecordExp {fields} ->
        let f ((name, e), d) =
          ...
          add ")"
        in
        add "RecordExp([" ; dolist d f fields ; add "])"
    | SeqExp l -> add "SeqExp[" ; dolist d exp l ; add "]"
    ...
    | ForExp {var= v; escape= b; lo; hi; body} ->
        add
        @@ concat
             ["ForExp("; string_of_symbol v; ","; string_of_bool !b; ",\n"] ;
        exp (lo, d + 1) ;
        add ",\n" ;
        exp (hi, d + 1) ;
        add ",\n" ;
  ...
  and dec (theDec, d) =
    ...
    | FunctionDec l ->
        let f (Fdecl {name; args; result; body; _}, d) =
          add @@ concat [indent d; "("; string_of_symbol name; ",["] ;
          dolist d (fun x -> add @@ field x) args ;
          add
          @@ concat
               [ "],\n"
               ; indent (d + 1)
  ...
  and ty (theType_, d) =
    concat [indent d; string_of_type ~max_unfolds theType_]
  ...
  and var_desc (e, d) =
    add @@ indent d ;
    match e with
    | SimpleVar s -> add @@ concat ["SimpleVar("; string_of_symbol s; ")"]
    | FieldVar (v, s) ->
        add "FieldVar(\n" ;
        var (v, d + 1) ;
        add ",\n" ;
...
let print_exp max_unfolds out e =
