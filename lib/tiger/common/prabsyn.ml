...
let as_string e0 =
  ...
  let rec dolist d f ls : unit =
    ...
    | [] -> ()
  in
  let rec exp (Exp {exp_base; _}, d) : unit =
    add @@ indent d ;
    match exp_base with
    | VarExp v ->
        add "VarExp(\n" ;
    ...
    | RecordExp {fields; typ; _} ->
        let f ((name, e), d) =
          ...
          add ")"
        in
        add @@ concat ["RecordExp("; string_of_symbol typ; ",["] ;
        dolist d f fields ;
        add "])"
    ...
    | FunctionDec l ->
        let f (Fdecl {name; params; result; body; _}, d) =
          ...
          @@ concat
               ...
               ; ( match result with
                 | None -> "NONE"
                 | Some (s, _) -> "SOME(" ^ string_of_symbol s ^ ")" )
               ; ",\n" ] ;
          exp (body, d + 1) ;
          add ")"
        in
