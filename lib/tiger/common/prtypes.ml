...
let rec asStringI unfolds max_unfolds seen ty =
  let asStringI' = asStringI (unfolds + 1) max_unfolds in
  (* increment of unfolds *)
  match ty with
  | T.RECORD (fields, _) ->
      let fieldString (s, ty) =
        concat [ string_of_symbol s; ": "; asStringI' seen ty ]
      in
      let fieldsString = String.concat ", " (List.map fieldString fields) in
      concat [ "RECORD {"; fieldsString; "}" ]
  | T.NIL -> "NIL"
  | T.INT -> "INT"
  | T.STRING -> "STRING"
