  ...
  let test (total, passes) c =
    ...
    let total, passes, out =
      try
        ...
        let out =
          ...
          Printf.printf "%sok!%s\n" green nc;
          flush stdout;
          (total + 1, passes + 1, Some (gotexit, gotout, goterr))
        in
        out
      with e ->
    ...
    let counts =
      try
        ...
        let gotexit, gotout, goterr =
          match out with
          | Some (gotexit, gotout, goterr) -> (gotexit, gotout, goterr)
          | None -> (expexit, expout, experr)
        in

        if sgotexit <> gotexit || sgotout <> gotout || sgoterr <> goterr then (
          Printf.printf "%sfailed!%s\n" red nc;
          if sgotexit <> gotexit then
            Printf.printf "  exit:\n    got %s, expected %s\n"
              (string_of_status sgotexit)
              (string_of_status gotexit);
          if sgotout <> gotout then (
            Printf.printf "  stdout:\n";
            Common.print_diff sgotout gotout);
          if sgoterr <> goterr then (
            Printf.printf "  stderr:\n";
            Common.print_diff sgoterr goterr);
          (total + 1, passes))
        else (
          Printf.printf "%sok!%s\n" green nc;
          flush stdout;
